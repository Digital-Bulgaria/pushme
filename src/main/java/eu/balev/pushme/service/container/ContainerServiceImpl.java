...
public class ContainerServiceImpl implements ContainerService{
	...
	public String createContainer(User owner) {
		...
		else
		{
			...
			synchronized(anonLock)
			{
				...
				LOGGER.debug("Currently there are {} anonymous containers... Maximum count which is allowed is {}.", 
					anonContainersCnt, 
					maxNoAnonContainers);
				
				pruneAnonymousContainers(
						anonContainersCnt.intValue(), 
						maxNoAnonContainers);
			}
		}
	...
	private void pruneAnonymousContainers(int totalCnts, int maxCnts)
	{
		if (totalCnts >= maxCnts)
		{
			...
			LOGGER.debug("We are going to prune {} anonymous containers.", cntrsToPrune);
			
			List<Container> containers = containerRepo.findOldestAnonymousContainers(
					new PageRequest(0, cntrsToPrune)
			);
		
			containers.forEach(c -> 
				{
					LOGGER.debug("Delete container with id {}.", 
							c.getId());
		...
		{
			LOGGER.debug("There is no need to prune the anonymous containers since the maximum "
					+ "limit is not yet reached. Total {}. Max {}.",
					totalCnts, 
					maxCnts);
		}
	}
